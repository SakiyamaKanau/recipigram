class RecipesController < ApplicationController
  ...
  def index
    ...
    ids = REDIS.zrevrangebyscore "recipes/daily/#{Date.today.to_s}", "+inf", 0,limit:[0,3]
    @ranking_recipes = ids.map{ |id| Recipe.find(id) }
  end
  
  def show
    @recipe = Recipe.find(params[:id])
    REDIS.zincrby "recipes/daily/#{Date.today.to_s}", 1, "#{@recipe.id}"
  end
  ...
  def destroy
    @recipe = Recipe.find(params[:id])
    REDIS.zrem "recipes/daily/#{Date.today.to_s}", @recipe.id
    @recipe.destroy
